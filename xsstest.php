			...
			<div class='news-list'>
				<!-- <div class='news-list-item clearfix'>
					<div class='col-xs-4'> -->
						...
						<!-- 以下为新加的留言板内容 -->
						<div class="title_textarea">
							<h1 style="color:;">留言板</h1><span>给作者的留言，作者审核完毕后将出现在留言区<span>
						</div>
						<div class="textarea">
							<form role="form" action=# method="get" name="talk_upload">
  								<div class="form-group">
							    <textarea class="form-control" rows="3" name="talk"></textarea>
							 	</div>
							 	<div class="container">     
									<button type="submit" class="btn btn-info btn-lg">提交留言</button>      
								</div>
							</form>
						</div>
						<!-- xss位置 -->
						...
						<?php
							...
							else
							{
								...
								$sql = 'select * from tb_talk limit '.$start.",".$every_item;
								$result = mysqli_query($conn,$sql);
								...
								while($row=mysqli_fetch_array($result)){
									...
									echo $row['title'];
									echo "</div>";
									echo "<div class='status'>日期:".$row['timeover']."</div>";
									echo "<div class='info'>".$row['talk']."</div>";
